...
class ComicRepositoryTest : TestCase() {
    ...
    fun get_comic_by_id() {
        runBlocking {
            Mockito.`when`(comicService.getComicData(1332, 1213223, "sdafsf44fff", "sdafsdsdfs")).thenReturn(Response.success(Gson().fromJson(successJson, ComicData::class.java)))
            val response = comicRepository.getComic(1332, 1213223, "sdafsf44fff", "sdafsdsdfs")
            assertEquals(Gson().fromJson(successJson,ComicData::class.java), response)
        }
    }
}